...
const getDeleteButton = (email) => {
    return `<button class="options-container-button" data-local-action="showActionBox action-box-collaborators ${email}">
...
export class CollaboratorsTab {
    ...
    async showActionBox(_target, componentName, email) {
        await assistOS.UI.showActionBox(_target, "", componentName, "append", {email});
    }
    ...
    async removeCollaborator(button, email) {
        ...
        if (confirmation) {
            let message = await spaceModule.removeCollaborator(assistOS.user.email, assistOS.space.id, email);
            if (message) {
                alert(message);
            } else {
                this.invalidate();
