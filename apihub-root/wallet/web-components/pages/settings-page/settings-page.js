...
export class SettingsPage {
    ...
    beforeRender() {
        ...
        for(let company of Object.keys(this.apiKeys)){
            ...
            if(this.apiKeys[company].value){
                stringHTML += `<svg class="pointer" data-local-action="deleteAPIKey ${company}" width="19" height="20" viewBox="0 0 19 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    async editKey(_eventTarget, type) {
        let confirmation = await assistOS.UI.showModal( "edit-apikey-modal", {type: type}, true);
        if(confirmation){
            this.invalidate(this.getAPIKeys);
        }
    }
