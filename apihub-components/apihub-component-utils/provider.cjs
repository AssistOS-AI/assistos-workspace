...
class Provider {
    ...
    async getTextResponse(model, prompt, options = {}) {
        ...
        const args = buildArgs({ subcommand: 'generateText', model, promptOrMessages: prompt, options })
    ...
    async getChatCompletionResponse(model, messages, options = {}) {
        if (!this.models[model]) throw new Error(`Provider ${this.name} does not support model ${model}`)
        const args = buildArgs({ subcommand: 'getChatCompletion', model, promptOrMessages: messages, options })
        const binariesPath = composeBinaryPath(this.name)
        return await Binaries.executeBinary(this.name, binariesPath, args)
    }
