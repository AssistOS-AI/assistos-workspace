...
async function addFileObject(request, response) {
    ...
    try {
        ...
        subscriptionManager.notifyClients(request.sessionId, objectType);
        subscriptionManager.notifyClients(request.sessionId, objectId);
...
async function deleteContainerObject(request, response) {
    ...
    try {
        ...
        await deleteContainerObjectTable(lightDBEnclaveClient, objectId);
        subscriptionManager.notifyClients(request.sessionId, objectId, "delete");
        subscriptionManager.notifyClients(request.sessionId, objectId.split('_')[0]);
        return utils.sendResponse(response, 200, "application/json", {
            success: true,
            data: objectId,
            message: `Object ${objectId} deleted successfully`
...
async function addEmbeddedObject(request, response) {
    ...
    try {
        ...
        subscriptionManager.notifyClients(request.sessionId, parts[parts.length - 2]);
        return utils.sendResponse(response, 200, "application/json", {
            success: true,
            data: objectId,
        ...
        if (propertyName) {
            ...
            if (Array.isArray(objectData) && objectData.length !== 0) {
                if (typeof objectData[0] === "object") {
                    ...
                    subscriptionManager.notifyClients(request.sessionId, objectId, propertyName);
                    return utils.sendResponse(response, 200, "application/json", {
                        success: true,
                        data: objectId,
                        message: `Object ${objectId} updated successfully`
...
async function addSpaceChatMessage(request, response) {
    ...
    try {
        ...
        subscriptionManager.notifyClients(request.sessionId, `chat_${spaceId}`);
    } catch (error) {
...
async function getChatTextResponse(request, response) {
    ...
    if (modelResponse.success) {
        ...
        for (const chatMessage of chatMessages) {
            ...
            subscriptionManager.notifyClients(request.sessionId, `chat_${spaceId}`, {}, [userId]);
        }
    }
}
