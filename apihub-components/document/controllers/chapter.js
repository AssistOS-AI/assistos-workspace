...
async function getChapter(req, res) {
    ...
    if (!spaceId || !documentId || !chapterId) {
        return util.sendResponse(res, 400, "application/json", {
            message: "Invalid request" + `Missing ${spaceId ? 'spaceId ' : ''}` + `Missing ${documentId ? 'documentId ' : ''}` + `Missing ${chapterId ? 'chapterId ' : ''}`,
            success: false
        });
    }
    try {
...
async function createChapter(req, res) {
    ...
    if(!spaceId ||!documentId ||!chapterData){
        return util.sendResponse(res, 400, "application/json", {
            message: "Invalid request" + `Missing ${spaceId ? 'spaceId ' : ''}` + `Missing ${documentId ? 'documentId ' : ''}` + `Missing ${chapterData ? 'request body' : ''}`,
            success: false
        });
    }
    try{
...
async function updateChapter(req, res) {
    ...
    if(!spaceId ||!documentId ||!chapterId ||!chapterData){
        return util.sendResponse(res, 400, "application/json", {
            message: "Invalid request" + `Missing ${spaceId ? 'spaceId ' : ''}` + `Missing ${documentId ? 'documentId ' : ''}` + `Missing ${chapterId ? 'chapterId ' : ''}` + `Missing ${chapterData ? 'request body' : ''}`,
            success: false
        });
    }
    try{
        await chapterService.updateChapter(spaceId, documentId, chapterId, chapterData);
        return util.sendResponse(res, 200, "application/json", {
            success: true,
