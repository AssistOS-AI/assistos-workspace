class ConfirmFlowExecution {
    ...
    async start(context) {
        try {
            ...
            let systemMessage = `There may be multiple applications installed in this system and you can find them here${JSON.stringify(context.spaceObjects)}. You have successfully executed the operation that has this description: ${flow.description} which had these necessary parameters: ${JSON.stringify(flow.parameters)}. You have executed the operation using these parameters: ${JSON.stringify(context.parameters)}. Redirect the user to the application responsible of the operation if the result of the operation is not visible here`;
